...
**不要把 Context Window 当作 RAM。** SWE-agent 和后续总结都强调：Harness 不只是"给更多上下文"，而是设计一个符合模型认知结构的 **Agent-Computer Interface (ACI)**。核心不是信息总量，而是信息如何被裁剪、排序和回流：

- **搜索结果必须限流**：超过阈值就要求 Agent 缩小查询范围，而不是返回 1000 行 `grep`
- **文件阅读必须有窗口化视图**：一次看局部片段，比整文件倾倒更可靠
- **编辑必须带即时反馈**：修改后立刻 lint / parse / typecheck，避免错误扩散
- **旧上下文必须压缩**：旧观察应总结成短摘要，而不是长期占用主上下文

这类设计看似细碎，但它们共同决定了 Agent 是在"有护栏的工作台"上工作，还是在"会不断把自己淹没的信息洪流"里工作。
...
**OpenAI 的进一步结论是：不只是给 Agent 浏览器，还要让整个应用对 Agent 可见。** 这叫 **Application Legibility（应用可读性）**。如果 Agent 只能读代码，却不能启动隔离环境、访问日志、指标、Trace、DOM 快照，那么它只能猜；一旦它能像人类工程师一样观察运行中的系统，验证和调试能力才真正闭环。

### 6.1.1 Application Legibility：让系统对 Agent 可见

OpenAI 的做法不是把浏览器当成一个额外工具，而是把"可运行、可观察、可隔离的应用实例"变成 Harness 的一部分：

- **每个任务在独立 git worktree 中运行**：避免不同任务互相污染运行状态
- **每个 worktree 都能独立启动应用**：Agent 不必争抢共享 dev server
- **Agent 可访问 DOM 快照、截图、浏览器导航**：用于复现和验证 UI 问题
- **Agent 可访问日志、指标、trace**：用于定位行为层面的问题，而不只读代码猜测

**原则**：代码可见性只解决"它写了什么"；应用可读性解决"它到底有没有工作"。

**最小实践建议**：

1. 让项目支持在独立 worktree 中一键启动
2. 为每个 worktree 提供独立端口和临时数据目录
3. 将日志和测试输出落盘，避免把大段输出直接塞进上下文窗口
4. 至少给 Agent 一种结构化观察 UI 的方式：Accessibility Tree、DOM 快照或带元素引用的浏览器工具
...
### 7.2.1 Anthropic 模式：Initializer Agent -> Coding Agent

对于跨多会话、长周期的大项目，单靠"每次启动先读 progress.json"还不够。Anthropic 的关键做法是把长任务拆成两个角色：

- **Initializer Agent**：只负责初始化工程环境，不直接开发功能
- **Coding Agent**：后续每个会话只处理一个具体 feature，并留下干净交接状态

Initializer Agent 的典型产出有三类：

1. `init.sh`：后续每个会话都可重复使用的环境启动脚本
2. `feature_list.json`：完整功能清单，每项都带测试步骤，初始状态为未完成
3. `progress` 文件 + 初始 git commit：为后续会话提供稳定锚点

这里最重要的不是文件名，而是 **"完成状态不能靠 Agent 从代码里猜"**。功能是否完成，必须有一个结构化、可验证、可持续更新的外部锚点。

### 7.2.2 Clean State Requirement：每次交接都要留下可继续工作的仓库

Anthropic 模式的另一条硬规则是：**每个会话结束时都必须留下 clean state**。也就是说，下一个 Agent 接手时，不该面对半成品、坏掉的 dev server、未记录的临时 workaround。

会话结束前至少满足：

- 当前改动已 commit，或明确回退到最近的可工作状态
- 相关测试已运行，失败项被记录
- `progress.json` / `feature_list.json` 已更新
- 下一个会话知道"接下来做什么"，而不是先做考古

如果做不到 clean state，就不要开始下一个 feature。跨会话 Agent 系统最常见的崩坏方式，不是模型变笨，而是把脏状态一层层传递下去。
...
### 8.1.1 棕地改造：如何把 Harness 加到旧代码库

不是所有团队都从 greenfield 开始。对遗留代码库，最大风险不是"没有 Harness"，而是**一口气加太多规则后被告警淹没**。这和第一次在老项目上打开静态分析很像。

推荐顺序：

1. **先补最短反馈环**：pre-commit、基础测试、最关键的 lint/typecheck
2. **再补状态恢复**：AGENTS.md、ADR、progress/feature 文件
3. **最后收紧架构约束**：只先约束最昂贵的跨层违规，而不是一次清理全部历史问题

**经验法则**：不要试图让旧代码库一夜之间"完全 harness-friendly"。先把最常见、最贵的失败模式机械化，等 Agent 在这些边界内稳定后，再继续收窄解空间。

### 8.1.2 Harness as Template：把 Harness 当作团队级服务模板

Martin Fowler 的一个有价值延伸是：Harness 很可能会演化成新的 **service template / golden path**。未来团队选型时，不只看框架是否流行，也会看：

- 这个技术栈有没有成熟的 Harness 模板
- 这个项目拓扑是否容易机械化约束
- 这套工具链是否同时对人类工程师和 Agent 友好

因此，组织级 Harness 的最佳形态往往不是"每个仓库从零设计"，而是：

- 提供一套可复用的 AGENTS.md / ADR / Hook / linter / CI 模板
- 允许团队在模板之上做项目特定校准
- 把项目中的有效修复持续回灌到模板，而不是只留在单个仓库里

这和传统 service template 的区别在于：Harness 模板不只是脚手架，它还包含反馈循环、约束系统和状态管理机制。
